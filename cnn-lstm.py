...
def build_cnn_lstm_model(input_shape):
    model = Sequential()
    model.add(Conv1D(128, kernel_size=9, activation='relu', input_shape=input_shape))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(512, kernel_size=9, activation='relu'))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(256, kernel_size=3, activation='relu'))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(128, kernel_size=3, activation='relu'))
    model.add(MaxPooling1D(pool_size=2))
    model.add(LSTM(256, return_sequences=True))  # 添加 LSTM 层，返回序列
    model.add(TimeDistributed(Dense(256, activation='relu')))  # 使用 TimeDistributed 包裹 Dense 层以处理序列数据
    model.add(Flatten())
    model.add(Dense(128, activation='relu'))
    model.add(Dropout(0.5))  # 添加 Dropout 层以防止过拟合
    model.add(Dense(4, activation='softmax'))  # 输出层，4 个类别
    return model
...
# 训练模型
model.fit(X_train, y_train, epochs=30, validation_data=(X_test, y_test), batch_size=256)
