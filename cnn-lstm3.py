...
# 重新构建模型，应用优化参数
def build_refined_cnn_lstm_model(input_shape, params):
    model = Sequential()
    model.add(Conv1D(64, kernel_size=params['kernel_size'], strides=2, activation='relu', padding='same', input_shape=input_shape))
    model.add(Conv1D(256, kernel_size=params['kernel_size'], strides=2, activation='relu', padding='same'))
...
model = build_refined_cnn_lstm_model(input_shape, optimized_params)
